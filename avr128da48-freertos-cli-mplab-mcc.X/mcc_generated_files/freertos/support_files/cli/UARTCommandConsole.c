...
xComPortHandle xCDCUsart = NULL; /* Static so it doesn't take up too much stack. */
	...
	vSerialPutString( xCDCUsart, (const signed char *) pcWelcomeMessage, strlen( pcWelcomeMessage ) );
	...
	for( ;; )
	{
        ...
        if (xSerialGetChar( xCDCUsart , (signed char *) &cRxedChar , comNO_BLOCK ) == pdTRUE)
        {
			...
			if( cRxedChar == '\n' || cRxedChar == '\r' )
			{
				...
				vSerialPutString( xCDCUsart, (const signed char *) pcNewLine, strlen( pcNewLine ) );
				...
				do
				{
					...
					vSerialPutString( xCDCUsart, (const signed char *) pcOutputString, strlen( pcOutputString ) );
				...
				vSerialPutString( xCDCUsart, (const signed char *) pcEndOfOutputMessage, strlen( pcEndOfOutputMessage ) );
			}
			else
			{
				if( cRxedChar == '\r' )
